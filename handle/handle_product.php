  <?php
    ...
    if (isset($_GET['update_product'])) {
        $id_product = $_GET['update_product'] ?? 0; // Lấy id sản phẩm từ URL
        $product = new Product($connect, $id_product); // Lấy thông tin sản phẩm từ database
    ...
    if (isset($_GET['page']) && $_GET['page'] == 'product') {
        ...
        $resultCate = $connect->query("select * from categories");
    ...
    if (isset($_GET['page']) && $_GET['page'] == 'add_product') {
        // Lấy danh mục sản phẩm
        $category = $connect->query("select * from categories");
    ...
    // Lấy 8 sản phẩm mới nhất cho trang home
    $result = $connect->query("SELECT * FROM products ORDER BY created_at DESC LIMIT 8");
    ?>